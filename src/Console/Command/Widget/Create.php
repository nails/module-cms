...
class Create extends Base
{
    ...
    protected function execute(InputInterface $oInput, OutputInterface $oOutput)
    {
        ...
        if (!is_dir(self::TPL_PATH)) {
            if (!mkdir(self::TPL_PATH, self::TPL_PATH_PERMISSION, true)) {
                $oOutput->writeln('');
                $oOutput->writeln('<error>Widget directory does not exist and could not be created</error>');
                $oOutput->writeln('<error>' . self::TPL_PATH . '</error>');
                return $this->abort($oOutput, self::EXIT_CODE_FAILURE);
            }
        } elseif (!is_writable(self::TPL_PATH)) {
            $oOutput->writeln('');
            $oOutput->writeln('<error>Widget directory exists but is not writeable</error>');
            $oOutput->writeln('<error>' . self::TPL_PATH . '</error>');
            return $this->abort($oOutput, self::EXIT_CODE_FAILURE);
        }
        ...
        try {
            ...
            $oOutput->writeln('<error>Error creating widget</error>');
    ...
    private function abort($oOutput, $exitCode = self::EXIT_CODE_FAILURE)
    {
        ...
        $oOutput->writeln($colorOpen . 'Aborting widget creation' . $colorClose);
        $oOutput->writeln('');
